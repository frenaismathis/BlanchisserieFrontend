...
export class AuthService {
  ...
  login(email: string, password: string): Observable<{
    ...
    return this.http.post<{
      user: User
    }>('http://localhost:5150/api/auth/login', { email, password }, { withCredentials: true })
      .pipe(
        tap({
          next: response => {
  ...
  revokeToken(): Observable<any> {
    return this.http.post<any>('http://localhost:5150/api/auth/revoke-token', {}, { withCredentials: true })
      .pipe(
        tap(response => {
  ...
  logout(): Observable<any> {
    return this.http.post<any>('http://localhost:5150/api/auth/logout', {}, { withCredentials: true })
      .pipe(
        tap(() => {
